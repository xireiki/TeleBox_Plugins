...
const CONFIG_KEYS = { PROXY_HOST: "waifu_proxy_host" } as const;
...
const dataPath = createDirectoryInAssets("waifu");
...
class WaifuConfigManager {
    ...
    private static async init(): Promise<void> {
        ...
        try {
            ...
            this.configPath = path.join(dataPath, "waifu_config.json");
            this.backupPath = path.join(dataPath, "waifu_config.backup.json");
...
// ==================== 帮助文本 ====================
const helpText = `🎨 <b>随机纸片人插件 (Waifu)</b>

<b>用法：</b>
<code>${mainPrefix}waifu [数量] [标签] [参数]</code>
...
<b>示例：</b>
• <code>${mainPrefix}waifu</code> - 随机1张
• <code>${mainPrefix}waifu mask</code> - 随机1张带遮罩
• <code>${mainPrefix}waifu 3 mask</code> - 3张带遮罩
• <code>${mainPrefix}waifu 萝莉 2</code> - 萝莉标签2张
...
async function sendImagesWithFallback(
    ...
    try {
        ...
        console.error("[waifu] 发送失败，降级处理:", error.message);
        // 降级：逐张发送
        for (const img of images) {
            try {
...
async function handleProxyCommand(
    ...
    if (!args.proxyHost) {
        const currentHost = await WaifuConfigManager.getProxyHost();
        await editHtmlMessage(msg, `🔗 当前反代: <code>${htmlEscape(currentHost)}</code>`);
        return;
    }

    await WaifuConfigManager.setProxyHost(args.proxyHost);
    await editHtmlMessage(msg, `✅ 反代已设置为 <code>${htmlEscape(args.proxyHost)}</code>`);
}

async function handleWaifuCommand(
    msg: Api.Message,
    client: any,
    args: ParsedArgs
): Promise<void> {
    ...
    const proxy = await WaifuConfigManager.getProxyHost();
...
class WaifuPlugin extends Plugin {
    cleanup(): void {}

    description = `随机纸片人插件\n\n${helpText}`;

    cmdHandlers: Record<string, (msg: Api.Message) => Promise<void>> = {
        waifu: async (msg: Api.Message): Promise<void> => {
            try {
                ...
                await handleWaifuCommand(msg, client, args);
            } catch (error: any) {
                console.error("[waifu] 插件错误:", error);
...
export default new WaifuPlugin();
